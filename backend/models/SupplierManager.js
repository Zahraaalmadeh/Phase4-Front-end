...
export const createSupplier = async (supplierData) => {
    try {
        ...
        if (!supplierData.supplierId || !supplierData.companyName || !supplierData.email || !supplierData.products) {
        ...
        if (existingSupplier) {
            return { success: false, message: 'Supplier with this ID or email already exists' };
        }
        ...
        return { success: true, data: savedSupplier, message: 'Supplier created successfully' };
    } catch (error) {
        return { success: false, message: error.message };
    }
};
...
export const updateSupplier = async (supplierId, updateData) => {
    try {
        ...
        return { success: true, data: updatedSupplier, message: 'Supplier updated successfully' };
    } catch (error) {
        return { success: false, message: error.message };
    }
};
...
export const getSupplierRating = async (supplierId) => {
    try {
        ...
        return { success: true, data: { supplierId, rating: supplier.rating, companyName: supplier.companyName } };
    } catch (error) {
        return { success: false, message: error.message };
    }
};
...
export const setSupplierAvailability = async (supplierId, availabilityData) => {
    try {
        ...
        if (availability) {
            ...
            const newAvailability = new SupplierAvailability({
                supplierId,
                ...availabilityData
            });
            availability = await newAvailability.save();
        }

        return { success: true, data: availability, message: 'Availability updated successfully' };
    } catch (error) {
        return { success: false, message: error.message };
    }
};
...
export const checkSupplierAvailability = async (supplierId, requestedDate) => {
    try {
        ...
        if (!availability) {
            return { success: false, message: 'Availability not found', isAvailable: false };
        }

        const isDateInRange = new Date(requestedDate) >= new Date(availability.availableFrom) &&
            new Date(requestedDate) <= new Date(availability.availableTo);

        const canAcceptMore = availability.currentOrders < availability.maxCapacity;
        const isAvailable = availability.status === 'available' && isDateInRange && canAcceptMore;
...
export const uploadDocument = async (supplierId, documentData) => {
    try {
        ...
        return { success: true, data: savedDocument, message: 'Document uploaded successfully' };
    } catch (error) {
        return { success: false, message: error.message };
    }
};
...
export const sendMessage = async (messageData) => {
    try {
        const newMessage = new SupplierMessage({
            ...messageData,
            createdAt: Date.now()
        });

        const savedMessage = await newMessage.save();
        return { success: true, data: savedMessage, message: 'Message sent successfully' };
    } catch (error) {
        return { success: false, message: error.message };
    }
};

export const getSupplierMessages = async (supplierId, recipientId) => {
    try {
...
export const getUnreadMessages = async (recipientId, supplierId) => {
    try {
        ...
        return { success: true, data: unreadMessages, count: unreadMessages.length };
    } catch (error) {
        return { success: false, message: error.message };
    }
};